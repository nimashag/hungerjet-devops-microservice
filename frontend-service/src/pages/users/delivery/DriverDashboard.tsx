...
import { fetchAvailableOrders, respondToAssignment } from "../../../services/deliveryService";
...
const DriverDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (error.message === "AUTH_TOKEN_MISSING" || error.response?.status === 401) {
  ...
  const handleResponse = async (orderId: string, action: 'accept' | 'decline') => {
    ...
    try {
      ...
      const message = action === 'accept' 
        ? 'Order accepted! You can view it in "My Deliveries"' 
      ...
      alert('Failed to respond to order: ' + (error.response?.data?.message || 'Unknown error'));
    }
  };
  ...
  return (
    <DriverLayout>
      <div className="p-6">
        <h2 className="text-3xl font-bold mb-6 text-indigo-600">Available Orders</h2>
        ...
        {orders.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-600 text-lg">No available orders at your location right now.</p>
            <p className="text-gray-500 mt-2">Check back soon or update your pickup location in your profile.</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {orders.map((order) => (
              <div key={order._id} className="border-l-4 border-indigo-600 bg-white p-6 rounded shadow-md hover:shadow-lg transition">
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="text-sm text-gray-500">PICKUP LOCATION</p>
                    <p className="text-lg font-semibold text-gray-800">{order.restaurantLocation}</p>
                  </div>
                  <div>
