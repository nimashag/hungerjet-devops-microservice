...
const DriverMyDeliveries = () => {
  ...
  const [modalAction, setModalAction] = useState<
    "PickedUp" | "Delivered" | "Cancelled" | null
  >(null);
  ...
  const confirmAction = (
    deliveryId: string,
    action: "PickedUp" | "Delivered" | "Cancelled",
  ) => {
  ...
  const handleConfirmedUpdate = async () => {
    ...
    try {
      ...
      setDeliveries((prev) =>
        prev.map((d) => {
          if (d._id === modalDeliveryId) {
            return { ...d, status: modalAction as Delivery["status"] };
          }
          return d;
        }),
      );
  ...
  const statusBadge = (status: Delivery["status"]) => {
    switch (status) {
      case "Assigned":
        return (
          <span className="inline-block bg-yellow-300 text-yellow-900 px-3 py-1 rounded-full text-xs font-semibold">
            Assigned
          </span>
        );
      case "PickedUp":
        return (
          <span className="inline-block bg-blue-300 text-blue-900 px-3 py-1 rounded-full text-xs font-semibold">
            Picked Up
          </span>
        );
      case "Delivered":
        return (
          <span className="inline-block bg-green-300 text-green-900 px-3 py-1 rounded-full text-xs font-semibold">
            Delivered
          </span>
        );
      case "Cancelled":
        return (
          <span className="inline-block bg-red-300 text-red-900 px-3 py-1 rounded-full text-xs font-semibold">
            Cancelled
          </span>
        );
      default:
  ...
  const getModalTitle = (action: typeof modalAction): string => {
    if (action === "PickedUp") {
    ...
    return (
      <div className="grid gap-6">
        {ongoingDeliveries.map((delivery) => (
          <div
            key={delivery._id}
            className="border-l-4 border-blue-600 bg-white p-6 rounded shadow-md hover:shadow-lg transition"
          >
            ...
            <div className="mb-4 p-3 bg-gray-50 rounded">
              <p className="text-sm text-gray-600">
                <strong>Delivery To:</strong> {delivery.deliveryAddress?.street}
                , {delivery.deliveryAddress?.city || delivery.deliveryLocation}
              </p>
            </div>
    ...
    return (
      <div className="grid gap-6">
        {completedDeliveries.map((delivery) => (
          <div
            key={delivery._id}
            className="border-l-4 border-gray-400 bg-gray-50 p-6 rounded shadow-sm"
          >
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <p className="text-sm text-gray-500 font-semibold">PICKUP</p>
